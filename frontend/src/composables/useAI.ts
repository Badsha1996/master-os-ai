...
export function useAI() {
  ...
  // ************************** COMPUTED **************************
  const accelerationEmoji = computed(() => 
    apiService.getAccelerationEmoji(acceleration.value)
  );
  ...
  async function streamChatMessage(
    ...
    try {
      error.value = null;
      await apiService.streamChatMessage(text, onChunk, temperature, maxTokens);
    } catch (err) {
      error.value = String(err);
