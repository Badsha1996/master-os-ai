...
const chatMessagesList = ref<ChatMessage[]>([
  { human: '', ai: 'Hey! How can I help you today?' }
])
...
const handleInput = async (): Promise<void> => {
  ...
  try {
    if (mode.value === 'chat') {
      ...
      // Check if there's a finish action in the steps
      let finalAnswer = res.result || ''
      
      // If no result but we have steps, look for finish action observation
      if (!finalAnswer && res.steps?.length) {
        const finishStep = res.steps.find((step: { action: { name: string } }) => 
        ...
        if (finishStep && finishStep.observation) {
          ...
          // Alternatively, get the last observation
...
<template>
  <div class="flex flex-col h-screen bg-gradient-to-br from-gray-50 to-gray-100 font-sans">
      ...
      <div class="max-w-3xl mx-auto space-y-6">
        <div v-for="(msg, index) in chatMessagesList" :key="index" class="group">
          ...
          <div v-if="msg.human" class="flex justify-end mb-2">
            <div class="max-w-2xl">
              <div class="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-4 rounded-2xl rounded-tr-none shadow-lg">
          ...
          <div v-if="msg.human || !msg.human" class="flex gap-4">
            ...
            <div v-if="msg.human" class="flex-1 max-w-2xl">
              <div class="bg-white border border-gray-200 rounded-2xl rounded-tl-none shadow-sm p-4">
                <!-- Loading State -->
                ...
                <!-- AI Response -->
                <div v-else>
                  <p class="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{{ msg.ai }}</p>

                  <!-- AGENT STEPS -->
                  <div v-if="msg.steps?.length" class="mt-4 pt-4 border-t border-gray-100">
                    <div class="flex items-center justify-between mb-3">
                      <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Reasoning Steps</h4>
                      <button
                      ...
                      >
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path v-if="!msg.showSteps" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    ...
                    <div v-if="msg.showSteps" class="space-y-2">
                      ...
                      >
                        <div class="flex items-start gap-3">
                          <div class="flex-shrink-0 w-6 h-6 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-xs font-semibold">
                            {{ i + 1 }}
                          </div>
                          <div class="flex-1 space-y-1.5">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                              <div>
                                ...
                                <p class="text-xs text-gray-700 mt-0.5 font-mono">{{ step.action.name }}({{ step.action.input }})</p>
                              </div>
                              <div>
      ...
      <div class="max-w-3xl mx-auto">
        <div class="relative">
          ...
          <button
            ...
            class="absolute right-3 top-1/2 transform -translate-y-1/2 w-10 h-10 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-full flex items-center justify-center transition-all duration-200 hover:shadow-lg hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 disabled:hover:shadow-none"
          >
